...
export async function POST(request: NextRequest) {
  try {
    ...
    if (recentSubmission.length === 0) {
      return NextResponse.json(
        { error: "No pending music submission found. Generate a song first!" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: "🎄 Beautiful! Your Christmas melody played successfully! +100 points!",
      score: 100,
      isVerified: true,
    });
  } catch (error) {
    ...
    return NextResponse.json(
      { error: "An error occurred while verifying your submission" },
      { status: 500 }
    );
  }
}
